...
public class Midi {
	...
	public static void seeInfo() {
		try {
			...
			for(int i=0; i<infos.length; i++) {
				...
				int nrec = md.getMaxReceivers();
				int ntr = md.getMaxTransmitters();
				System.out.println(i + " " + infos[i] + " IN:" + ntr + " OUT:" + nrec);
			}
		} catch(Exception e) {
	...
	public static Sequence getSequence(URL file) {
		try {
			Sequence sequence = MidiSystem.getSequence(file);
			return(sequence);
		} catch(Exception e) {
	...
	public static Vector<Event<MidiMsg>> getEvents(Sequence sequence) {
		...
		for( Track track : sequence.getTracks() ) {
			...
			for(int i=0; i<n; i++) {
				...
				r.add(Event.make(event.getTick(), new MidiMsg(event.getMessage())));
			}
		}
		r.sort(Event.comparator);
		convertToMs(r, sequence.getResolution(), 500000);
		return(r);
	}

	static void convertToMs(Vector<Event<MidiMsg>> events, long ticksPerQuarter, long microsPerQuarter) {
		long lastTempoTick = 0;
		long lastTempoTime = 0;
		for(Event<MidiMsg> event : events) {
			long tick = event.time;
			long deltaTick = tick-lastTempoTick;
			long time = lastTempoTime + ((deltaTick * microsPerQuarter) / (1000 * ticksPerQuarter));
			event.time = time;
			int n = event.value.changeTempo();
			if(n > 0) {
				microsPerQuarter = n;
				lastTempoTick = tick;
				lastTempoTime = time;
	...
	public static Vector<Event<MidiMsg>> getEvents(File file) {
		return(getEvents(getSequence(file)));
	}

	public static Vector<Event<MidiMsg>> getEvents(URL url) {
		return(getEvents(getSequence(url)));
	}
	...
	public static void transmit(Transmitter trans, LinkedBlockingQueue<MidiMsg> queue) {
		trans.setReceiver(new Receiver() {
			public void close() {
				System.out.println("Closing receiver");
				trans.close();
	...
	/*
	 * Test for writing a Mifi file from Karl Brown
	 */
