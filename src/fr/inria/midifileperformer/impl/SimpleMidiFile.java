...
public class SimpleMidiFile {
	...
	static MidiMsg toMidi(String s) {
		...
		if(begin) return(MidiMsg.NoteOn(1, note, 100));
		return(MidiMsg.NoteOff(1, note, 0));
	}
	...
	public static Vector<Event<MidiMsg>> convert(String[] v) {
		Vector<Event<MidiMsg>> r = new Vector<Event<MidiMsg>>();
		for(String line : v)
			r.add(parseLine(line));
		return(r);
	...
	public static Vector<Event<MidiMsg>> demov() {
		return(convert(symbDemo));
	}
	
	static String[] symbDemo = new String[] {
